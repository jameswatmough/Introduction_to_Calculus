...
The best approach to sketching functions is to evaluate the function at a few points.  The trick is picking the best points. Since we know the cosine is zero when its input is a multiple of $\pi$, we pick values for $x$ for which $2x-3$, the input to the cosine, is $\pi$.
That is, solve $2x-3 = \pi$ for $x$.  We find $x = \frac{1}{2}(3+\pi)$.  Of course this is only one muliple of $\pi$.  We said any multiple, so replace $\pi$ by $n\pi$ and let $n$ be any integer.  Similarly, we know the cosine is 1 if its input is $\frac{\pi}{2}$ plus any multiple of $\pi$.  Let's put this all in a table for easy reference.  We'll call the input to the cosine the angle $\theta$, pick values for $\theta$, and then compute $x = \frac{1}{2}(3+\theta)$ and $y = 2 + 3\cos \theta$.
